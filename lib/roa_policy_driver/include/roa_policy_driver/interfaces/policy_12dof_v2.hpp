/*
POLICY INTERFACE: 12DOF V2
- Gravity observaiton added (3 floats)
*/
#pragma once
...
struct Policy12DofV2 {
  static constexpr int kDof = 12;
  static constexpr int kObsDim = 45;  // 3 + 3 + 12 + 12 + 3 + 12
  ...
  struct Obs {
    std::array<float, 3> gravity{};             // [gx, gy, gz] gravity unit vector in body frame
  ...
  static inline void pack_obs(const Obs& o, float* out36) noexcept {
    float* p = out36;
    std::memcpy(p, o.gravity.data(), sizeof(float) * 3); p += 3;
  ...
  static inline void unpack_obs(const float* in36, Obs& o) noexcept {
    const float* p = in36;
    std::memcpy(o.gravity.data(), p, sizeof(float) * 3); p += 3;
  ...
  static inline std::string default_model_path() {
    return std::string(ROA_POLICY_DRIVER_SHARE_DIR) + "/onnx/12dof_guv_v3/policy.onnx";
  }
